...
Note this value is not specified according to the key but the path segement. So if the key of the child route is `awesomeUser`, the `$exact` value should be `awesome-user` with default configuration.
...
To make the segment a segement parameter, we may add `$match` option as regular expression:
...
Please note that the regular expression must match the whole string of a segement. Assuming the path is `/settings-suffix`, even though `/(?:user-)?settings/.test('settings-suffix')` is true, but it won't match the route because it matches only `settings` instead of the whole segment `settings-suffix`.
